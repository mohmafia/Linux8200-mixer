...
@export var my_msg: String = "lvl_R"
...
func _initialize_leds():
	for i in range(1, 13):
		var node = get_node_or_null("Pvugreen_R" + ("" if i == 1 else str(i)))
		if node: all_leds.append(node)
	for i in range(1, 11):
		var node = get_node_or_null("Pvuyellow_R" + ("" if i == 1 else str(i)))
		if node: all_leds.append(node)
	for i in range(1, 7):
		var node = get_node_or_null("Pvured_R" + ("" if i == 1 else str(i)))
		if node: all_leds.append(node)
