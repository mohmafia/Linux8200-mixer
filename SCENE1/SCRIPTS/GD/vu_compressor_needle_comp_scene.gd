...
var min_angle = 45  # Minimale hoek (-20 dB) zet positief voor compressor
var max_angle = -45  # Maximale hoek (+5 dB) zet negatief voor compressor
var bpm = 150  # BPM van de techno beat
...
var damping_factor = 0.05  # Hoeveelheid demping (lager = trager)

func _process(delta):
	time_elapsed += delta

	# Simuleer een techno beat met een sinusgolf
	var beat_strength = abs(sin(time_elapsed * bpm * 0.0523)) * 1.3 

	# Voeg random fluctuatie toe voor realisme
	var noise = randf() * 0.12 - 0.1  
